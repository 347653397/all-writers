...
<div class="x_panel">
		...
		<div class="x_title">
			<h2><?=$current['title']??''?></h2>
            <a class="btn btn-primary pull-right" href="addCategory" cust="button">新增标签</a>
						...
						<?php if(!empty($categorys)) :?>
							<?php foreach ($categorys as $category) :?>
								<tr>
                                    ...
                                    <td><?=$category['sort'];?></td>
									<td>
										<a href="editCategory?id=<?= $category['id'];?>" class="btn btn-primary btn-xs" title="修改">
											修改
										</a>
                                        <?php if($category['status'] == '显示') :?>
                                            <a href="javascript:void;" data-url="/Admin/Setting/delCategory?id=<?= $category['id'];?>" class="btn btn-danger btn-xs del" title="删除">
