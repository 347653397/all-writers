...
<div class="x_panel">
		...
		<div class="x_title">
			<h2><?=$current['title']??''?></h2>
            <a class="btn btn-primary pull-right" href="add" cust="button">新增课程</a>
            <div class="x_content">
						...
						<?php if(!empty($courses)) :?>
							<?php foreach ($courses as $course) :?>
								<tr>
                                    ...
                                    <td><?=$course['status'];?></td>
									<td>
										<a href="edit?id=<?= $course['id'];?>" class="btn btn-primary btn-xs" title="修改">
											修改
										</a>
										<a href="audioList?course_id=<?= $course['id'];?>" class="btn btn-primary btn-xs" title="内容管理">
											内容管理
										</a>
                                        <?php if(empty($course['is_recommend']) || $course['is_recommend'] =='1') :?>
    				                        <a href="javascript:void(0)" data-url="/Admin/Course/recommend?id=<?= $course['id'];?>&is_recommend=2;?>" class="btn btn-primary btn-xs recommend" title="推荐">
    											推荐
    										</a>
                                        <?php else :?>
                                            <a href="javascript:void(0)" data-url="/Admin/Course/recommend?id=<?= $course['id'];?>&is_recommend=1;?>" class="btn btn-primary btn-xs recommend" title="推荐">
                                                取消推荐
                                            </a>
                                        <?php endif?>
										<a href="/Admin/Order/courseList?course_id=<?= $course['id'];?>" class="btn btn-primary btn-xs" title="购买信息">
											购买信息
										</a>
										<a href="/Admin/Comment/index?course_id=<?= $course['id'];?>" class="btn btn-primary btn-xs" title="评论列表">
											评论列表
										</a>
                                        <?php if($course['status'] == '显示') :?>
                                            <a href="javascript:void;" data-url="/Admin/Course/del?id=<?= $course['id'];?>" class="btn btn-danger btn-xs del" title="删除">
