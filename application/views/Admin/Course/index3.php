						...
						<?php if(!empty($courses)) :?>
							<?php foreach ($courses as $course) :?>
								<tr>
                                    ...
                                    <td><?=$course['sort'];?></td>
                                    <td><?=$course['status'];?></td>
                                    <td><?=$course['auction_status'];?></td>
									<td>
<!--										<a href="/Admin/Audio/edit?id=--><?//= $course['id'];?><!--" class="btn btn-primary btn-xs" title="修改">-->
<!--											修改-->
<!--										</a>-->
                                        <a href="javascript:void(0)" data-id="<?= $course['id']; ?>"  data-url="/Admin/Course/verifyAuction?id=<?= $course['id'];?>&type=1" class="btn btn-primary btn-xs verify" title="审核">
                                            拍卖审核
                                        </a>
    ...
    $(function(){
        $('.verify').on('click',function(){
            ...
            layer.confirm('拍卖审核', {
                ...
                $.ajax({
                    url: '/Admin/Course/verifyAuction',
                    type: 'post',
                    dataType: 'json',
                    data: {'id':id,'type':1},
                    success: function (res) {
                        if(res.status){
                            ...
                            setInterval(function () {
                                window.location.href = '/Admin/Course/auction';
                            },2000)
                        }else{
                            layer.msg(res.msg);
                        }
