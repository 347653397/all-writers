        ...
        <div class="x_panel">
            <div class="x_content">
                <form class="form-horizontal form-label-left select_from" action="edit" method="post" enctype="multipart/form-data" id="autoForm" style="font-size: 15px;font-weight: normal">
                    <span class="section">编辑音频</span>
                    <?php if ($audio['is_tg'] == '2'): ?>
                    <div class="item form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="price">拍卖价格 <span class="required">*</span></label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <input  class="form-control col-md-7 col-xs-12" name="auction_money" value="<?=$audio['auction_money']??''?>" type="text">
                        </div>
                    </div>
                    <div class="item form-group">
                        <label for="icon" class="control-label col-md-3 col-sm-3 col-xs-12">拍卖状态</label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <select id="heard" class="form-control" name="auction_status" >
                                <option value="">请选择..</option>
                                <option value="1" <?= $audio['auction_status'] == '1'?'selected = "selected" ':''?>>未申请</option>
                                <option value="2" <?= $audio['auction_status'] == '2'?'selected = "selected" ':''?>>申请中，待审核</option>
                                <option value="3" <?= $audio['auction_status'] == '3'?'selected = "selected" ':''?>>审核失败，竞拍失败</option>
                                <option value="4" <?= $audio['auction_status'] == '4'?'selected = "selected" ':''?>>审核成功，竞拍中</option>
                                <option value="5" <?= $audio['auction_status'] == '5'?'selected = "selected" ':''?>>竞拍结束</option>
                            </select>
                        </div>
                    </div>
                    <?php endif; ?>
                                ...
                                <div class="col-sm-3" style="margin: 0px -10px;">
                                    <div class="fileupload fileupload-new" data-provides="fileupload">
                                        <div class="fileupload-new thumbnail" style="height:auto">
                                            <img src="<?= $audio['audio_pic']??'/public/img/no_image.png'?>" alt="">
                                        </div>
                    ...
                    <div class="item form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <div class="col-sm-3" style="margin: 0px -10px;">
                                <div class="fileupload <?= $audio['audio_addr']?'fileupload-exists':'fileupload-new'?>" data-provides="fileupload">
                                    ...
                                    <div class="fileupload-preview fileupload-exists thumbnail" style="height: 50px; width: 300px;">
                                        <?php if($audio['audio_addr']):?> <audio src="<?= $audio['audio_addr'];?>" controls="controls"></audio> <?php endif; ?>
                                    </div>
                                    <span class="btn btn-white btn-file" style="border-color: rgba(150,160,180,0.3)">
                                            <span class="fileupload-new">
                            ...
                            <select id="heard" class="form-control" name="cid" required="">
                                ...
                                <?php if(!empty($category)) :?>
                                    <?php foreach ($category as $key => $tag) :?>
                                        <option value="<?= $key;?>" <?php if($audio['cid'] == $key) {echo 'selected="selected"';} ?>><?= $tag??'';?></option>
                                    <?php endforeach;?>
                                <?php endif;?>
                            </select>
                        </div>
                    ...
                    <div class="item form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <input id="title" class="form-control col-md-7 col-xs-12" name="title" value="<?=$audio['title']??''?>" type="text" required>
                        </div>
                    </div>
                    <div class="item form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12" for="author">作者 <span class="required">*</span></label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <input id="author" class="form-control col-md-7 col-xs-12" name="author" value="<?=$audio['author']??''?>"  type="text" required>
                        </div>
                    </div>
                    <div class="item form-group">
                        <label for="icon" class="control-label col-md-3 col-sm-3 col-xs-12">所属课程</label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <select id="heard" class="form-control" name="course_id" >
                                <option value="">请选择..</option>
                                <?php if(!empty($courses)) :?>
                                    <?php foreach ($courses as $key => $course) :?>
                                        <option value="<?= $key;?>" <?php if($audio['course_id'] == $key) {echo 'selected="selected"';} ?>><?= $course??'';?></option>
                                    <?php endforeach;?>
                                <?php endif;?>
                            </select>
                        </div>
                    ...
                    <div class="item form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <select  id="type" class="form-control" name ="type" required="">
                                <option value="">请选择..</option>
                                <option value="1" <?= $audio['type'] == '1'?'selected = "selected" ':''?>>收费</option>
                                <option value="2" <?= $audio['type'] == '2'?'selected = "selected" ':''?>>试听</option>
                            </select>
                        </div>
                    </div>
                    <div class="item form-group dj">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <input  class="form-control col-md-7 col-xs-12" name="price" value="<?=$audio['price']??''?>" type="text" hidden required>
                        </div>
                    </div>
                    <div class="item form-group">
                        <label for="icon" class="control-label col-md-3 col-sm-3 col-xs-12">是否原创</label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <select id="heard" class="form-control" name="is_original" required="">
                                <option value="">请选择..</option>
                                <?php if(!empty(is_original())) :?>
                                    <?php foreach (is_original() as $key => $tag) :?>
                                        <option value="<?= $key;?>" <?php if($audio['is_original'] == $key) {echo 'selected="selected"';} ?>><?= $tag??'';?></option>
                                    <?php endforeach;?>
                                <?php endif;?>
                            </select>
                        </div>
                    ...
                    <div class="item form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <textarea rows="4" class="resizable_textarea form-control" name="audio_brief" placeholder="" data-parsley-id="16"><?=$audio['audio_brief']??''?></textarea>
                        </div>
                    </div>
                    <div class="item form-group">
                    ...
                    <div class="item form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            <input  class="form-control col-md-7 col-xs-12" name="sort" value="<?=$audio['sort']??''?>" type="number" required>
                        </div>
                    </div>
                    ...
                    <div class="itme form-group">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                                <label>
                                    <input name="status" type="checkbox" class="js-switch" <?=$audio['status'] == '1'?'checked':''?> data-switchery="true" style="display: none;"><small style="left: 12px; transition: background-color 0.4s, left 0.2s; background-color: rgb(255, 255, 255);">
                                </label>
                        </div>
                    </div>
                    <div class="item form-group">
