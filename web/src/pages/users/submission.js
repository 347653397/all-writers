...
* @method submission 文件名
* @param { ReactElement } submission 组件名
...
class Submission extends Component{
    componentDidMount() {
        ...
        this.props.dispatch({type:'release/save',payload:{releaseRouter:'/users/submission'}})
    }
            ...
            <ul>
                {
                    subData && subData.map((d, i) => {
                        return (
                            <li key={i} >
                            <SwipeAction
						      disabled={d.audit_status!=2 && d.audit_status==1 || d.audit_status!=2 && d.audit_status==3 || d.audit_status==2 && d.auction_status==3?false:true}
						      right={[
						           {
							          text: '编辑',
						    ...
						    <div>
                                ...
                                <div className="right">
                                    <h3 onClick={() => pageChangeHandler(d.up_type == 2 ? d.course_id : d.id, d.up_type == 2 ? '/detai/list' : '/detai')} className="textOversTab2">{d.up_type == 2 ? <span className="color1">连载</span> : <span className="color2">单集</span>}{d.title}</h3>
                                    <p className="textOversTab2">{d.audio_brief || '暂无介绍'}</p>
                                    <p >{(()=>{
                                    	switch(Number(d.audit_status)){
                                    		case 1: return <span className="price" style={{ float: 'left', fontSize: '0.35rem' }}>待审核</span>
                                            break;
                                    		case 2:return <span className="price" style={{ float: 'left',color:'#999', fontSize: '0.35rem' }}>审核通过</span>;
                                    		break;
                                    		default:return <span className="price" style={{ float: 'left',color:'red', fontSize: '0.35rem' }}>审核未通过</span>;
                                    		break;
                                    	}
                                    })()}
                                      {
                                      	d.audit_status==3?
                                      	<font className="price" style={{ float: 'right',color:'red', fontSize: '0.35rem' }} onClick={()=>alert('未通过原因', d.audit_fail, [
								          { text: '取消', onPress: () => console.log('cancel') },
                                    ...
                                    <div className="tags">
                                        <p>{new Date(Number(d.create_time*1000)).Format("MM-dd hh:mm")}
                                        </p>
                                    </div>
                                </div>
                                </div>
                ...
                }
                {
                    subData && subData.length == 0 && !loading.effects['users/myCourse'] ?
                      <div>
                        ...
                        <p className="lengthIcontext">暂无投稿</p>
                       </div>
                        : null
                }
            </ul>
